...
- Treat Markdown in the repository as the only presentation source. Never hand-edit or commit `.pages-src/` or `.pages-site/`; they are generated by `scripts/build_pages.sh`.
- Keep `mkdocs.yml` navigation symmetric across the English and German paths. Run a strict Pages build after navigation or presentation-layer changes.
- Keep repository and Pages content synchronous: every public presentation change is committed first, Pages must deploy that exact commit, and a failed deployment blocks publication status.
- Never bypass required pull requests, CODEOWNER review, publication checks, fork-workflow approval, or default-branch protections. Do not grant write access to simplify a contribution.
- Keep pull-request workflows read-only and secret-free; do not introduce privileged or indirect PR triggers, mutable action tags, or self-hosted runners for untrusted changes.
